...
- Add `InfoClient.extra_agents()` with typed API-wallet responses.
- Add `reversed=True` to `InfoClient.user_fills()` time-range queries to request
  the newest fills first; `start_time` is required when enabled.

### Fixed

- Cover ascending and mixed-order fill responses to detect unintended client-side
  sorting, and verify reversed fill queries against complete reference windows.
